...
if [ $USER == "root" ]; then
	...
	if [[ $PWD != *"wan"* ]]; then
		    echo -e "${Yellow}Can not run from non 'wan' folder folder${Color_Off}"
            build_exit 80 $PWD
	fi
    ...
    if [[ $PWD != *"template"* ]]; then
            echo -e "${Yellow}Can not run from non 'template' folder folder${Color_Off}"
            build_exit 81 $PWD
    fi
...
read -p 'verify password for which user name: ' CHANGE_USER

#htpasswd -v $BCRYPT  $HTPASSWD_PATH $CHANGE_USER
htpasswd -v $HTPASSWD_PATH $CHANGE_USER
