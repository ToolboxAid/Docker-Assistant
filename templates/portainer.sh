...
create_portainer          $DOCKER_PATH/templates/${DEPLOYING_FOLDER}/.env
...
echo "mkdir $DOCKER_PATH/$ENV/$SITE/data"
...
#----------------------------------------------------------
sed -i "s/~SITE~/$SITE/g"                        $DOCKER_PATH/$ENV/$SITE/.env
sed -i "s/~SITE2~/$SITE2/g"                      $DOCKER_PATH/$ENV/$SITE/.env
